...
const interactionLibrary: Partial<Record<PeptideId, string[]>> = {
  "bpc-157": [
    "BPC-157 can usually share the same recovery injection with TB-500 when both are part of the research protocol.",
    "If KLOW or GLOW is already in the stack, standalone BPC-157 is redundant and should be removed.",
  ],
  "tb-500": [
    "TB-500 is commonly paired with BPC-157 in the same recovery window.",
    "If KLOW or GLOW is already in the stack, standalone TB-500 is redundant and should be removed.",
  ],
  "ghk-cu": [
    "GHK-Cu is usually better kept as its own topical routine instead of being forced into a GH-axis injection schedule.",
    "If KLOW or GLOW is already in the stack, separate standalone GHK-Cu is usually unnecessary unless there is a deliberate topical add-on reason.",
  ],
...
const basePlans: Record<GoalOption, { stackName: string; summary: string; primary: PeptideId[]; enhanced: PeptideId[]; premium: PeptideId[]; longTermAdds: PeptideId[]; conditionAdds: Partial<Record<HealthCondition, PeptideId[]>>; rationale: Partial<Record<PeptideId, string>>; genericComparison: string[] }> = {
  "Pain & Injury Recovery": {
    ...
    summary: "For recovery goals, the engine starts with repair-first literature, then decides whether a convenience blend or standalone components make more sense based on the research intake.",
    primary: ["bpc-157", "tb-500"],
    enhanced: ["klow", "mots-c"],
    premium: ["klow", "nad-plus", "mots-c"],
    longTermAdds: ["mots-c"],
    conditionAdds: { "Digestive stress or gut sensitivity": ["bpc-157"], "Slow workout recovery": ["mots-c"] },
    rationale: {
      klow: "KLOW provides a convenience-first blend of BPC-157 + TB-500 + GHK-Cu + KPV, reducing the need for separate recovery vials in research protocols.",
      "bpc-157": "BPC-157 appears as a standalone when the intake clearly points away from a containing blend or references prior BPC-157 research directly.",
      "tb-500": "TB-500 pairs with BPC-157 as the foundational recovery research stack.",
      "mots-c": "MOTS-C appears when recovery research goals extend to metabolic resilience and mitochondrial support.",
      "nad-plus": "NAD+ broadens the recovery research framework into cellular energy and repair pathways.",
    },
    genericComparison: [
      "A generic recovery result would show everyone BPC-157 and TB-500 side by side. This version decides whether KLOW or GLOW should replace the individual parts based on the research intake.",
      "The engine removes redundant standalone peptides whenever a blend already contains them, keeping the research stack clean.",
    ],
  },
...
function applyBlendPreference(peptides: PeptideId[], answers: QuizAnswers, goal: GoalOption): PeptideId[] {
  ...
  if (goal === "Pain & Injury Recovery") {
    ...
    if (preference === "Individual peptides (maximum control)") return unique(peptides.filter((id) => !["klow", "glow"].includes(id)).concat(["bpc-157", "tb-500"]));
...
function applyAdjustments(base: PeptideId[], answers: QuizAnswers, goal: GoalOption) {
  ...
  let peptides = [...base];
  if (answers.priority === "Highest-confidence research first") peptides = unique([...plan.primary, ...peptides]);
...
function buildRouteAdjustment(id: PeptideId, answers: QuizAnswers) {
  ...
  if (route === "Subcutaneous injection (highest bioavailability)") return "Subcutaneous injection is the most commonly studied route in published peptide research, so the engine kept the strongest research-fit options on the table.";
...
function buildResearchNotes(answers: QuizAnswers, goal: GoalOption, isFull: boolean) {
  const notes = [
    ...
    "Every dosing mention is framed as published research or educational protocol language, not a personal recommendation.",
    `This result was adjusted for ${goal.toLowerCase()} research goals, plus the constraints shared around budget, sensitivity, and administration style.`,
...
function buildWarnings(peptides: PeptideId[]) {
  return unique(peptides.flatMap((id) => peptideProfiles[id].warnings).concat([
    ...
    "If pregnancy, breastfeeding, cancer history, psychiatric medication use, endocrine medication use, or anticoagulation is part of the picture, a qualified healthcare provider should review any research protocol context first.",
  ]));
}
...
export function buildResult(answers: QuizAnswers, isFull: boolean): QuizResult {
  ...
  return {
    headline: isFull ? `Your ${goal.toLowerCase()} research stack report` : `Your ${goal.toLowerCase()} research overview`,
    ...
    educationalLinks,
    reportIntro: "This report compiles publicly available research into a clear, practical framework tailored to the signals you provided. All compounds are presented for educational and research purposes only.",
